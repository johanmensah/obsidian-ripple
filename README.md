...
- **Scoped reflections** can consider one note, a thread up to a selected point, one terminal branch, or the whole tree. Prompts retain note times and parent relationships, and an in-progress reflection returns when Ripple is reopened.
...
- **Cleaner mobile controls** keep Reply available on every note, reserve inline reflection actions for terminal notes, and submit compact composers with Enter.
