...
- **Feed**: newest-first under day headers, composer pinned on top. `n` focuses the composer, `Cmd/Ctrl-Enter` posts, `j`/`k` move, `Enter` edits in place, `r` replies, `h` cycles highlights, `o` opens the post as a normal note.
